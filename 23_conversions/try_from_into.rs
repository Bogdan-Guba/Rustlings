...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let convert_component = |val: i16| -> Result<u8, IntoColorError> {
            if val < 0 || val > 255 {
                return Err(IntoColorError::IntConversion);
            }
            Ok(val as u8)
        };
        let red_val = convert_component(tuple.0)?;
        let green_val = convert_component(tuple.1)?;
        let blue_val = convert_component(tuple.2)?;
        Ok(Color {
            red: red_val,
            green: green_val,
            blue: blue_val,
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        ...
        let red_val = convert_component(arr[0])?;
        let green_val = convert_component(arr[1])?;
        let blue_val = convert_component(arr[2])?;
